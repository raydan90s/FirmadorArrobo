# NotaDebitoController — Migración al estándar FacturacionController

## Cambios realizados

### `Models/NotaDebitoRequest.cs`
...
### `Controllers/NotaDebitoController.cs`

#### Bloques eliminados
- **Test endpoint** (`GET /api/notadebito/Test`) — FacturacionController no tiene test endpoint.
...
- **CorregirDecimales** — Aplica formato `F2` a nodos monetarios antes de firmar:
  - `//totalSinImpuestos`, `//valorTotal`, `//baseImponible`, `//valor`
...
- Construcción del modelo dominio: `Emisor`, `Establecimiento`, `PuntoEmision`, `DocumentoModificado`, `Motivos`, `NotaDebito_1_0_0Modelo.NotaDebito`.
- Generación de clave de acceso.
- Serialización XML + firma XAdES.
- Generación de PDF vía `_rideService.NotaDebito_1_0_0`.
...
3. Construir modelo dominio (Emisor, DocumentoModificado, Motivos, InfoNotaDebito)
4. DateTimeOffset.TryParse(FechaEmision) → DateTime con offset -05
5. Serializar XML vía NotaDebito_1_0_0Mapper
6. CorregirFecha: //fechaEmision = dd/MM/yyyy
7. CorregirDecimales: F2 en 4 campos monetarios
...
| Aspecto | FacturacionController | NotaDebitoController |
|---------|----------------------|-----------------------|
| Modelo documento | `Factura_1_0_0Modelo` | `NotaDebito_1_0_0Modelo` |
| Mapper | `Factura_1_0_0Mapper` | `NotaDebito_1_0_0Mapper` |
| TipoDocumento | `Factura_1_0_0Modelo.TipoDocumento` | `NotaDebito_1_0_0Modelo.TipoDocumento` |
| Info documento | `InfoFactura` (TotalDescuento, ImporteTotal, TotalConImpuestos, Pagos) | `InfoNotaDebito` (TotalSinImpuestos, ValorTotal, Impuestos, Pagos) |
| DocumentoModificado | No tiene | `DocumentoSustento` (CódDocumento, NumDocumento, FechaEmisionDocumento) |
| Motivos | No tiene | `Motivos` (lista de Razón + Valor) |
| Detalles | Con subsidio (`MapearDetallesConSubsidio`) | No tiene |
| CorregirDecimales | `//totalSinImpuestos`, `//totalDescuento`, `//baseImponible`, `//valor`, `//importeTotal`, `//propina`, `//total` + detalle | `//totalSinImpuestos`, `//valorTotal`, `//baseImponible`, `//valor` (solo documento) |
| Nombre archivos | `FACTURA_...` | `NOTADEBITO_...` |
| RIDE generator | `_rideService.Factura_1_0_0` | `_rideService.NotaDebito_1_0_0` |

El patrón de certificado, logo, fecha, decimales, logging SRI, autorización y base64 output es **idéntico** en todos los controllers.

---

## Guía para el backend — Ejemplo de request

**Endpoint:** `POST /api/notadebito/GenerarNotaDebito`
...
| `cliente.*` | Sí | `identificacion`, `razonSocial` y `tipoIdentificador` (`"RUC"`, `"CEDULA"`, `"PASAPORTE"`) |
| `infoNotaDebito.*` | Sí | Totales, impuestos y pagos |
| `infoNotaDebito.totalSinImpuestos` | Sí | Suma de los valores sin impuestos |
| `infoNotaDebito.valorTotal` | Sí | Valor total de la nota de débito (incluye impuestos) |
| `infoNotaDebito.impuestos` | Sí | Lista de impuestos aplicados. Cada uno con `codigo`, `codigoPorcentaje`, `baseImponible`, `tarifa`, `valor` |
| `infoNotaDebito.pagos` | Sí | Al menos una forma de pago. Cada uno con `formaPago`, `total`, `plazo` opcional, `unidadTiempo` opcional |
| `documentoModificado.*` | Sí | `codDocumento` (ver tabla abajo), número y fecha del documento original |
| `motivos` | Sí | Al menos un motivo con `razon` (texto) y `valor` (decimal) |
...
### Códigos de forma de pago (`pagos[].formaPago`)
...
{
  "success": true,
  "claveAcceso": "3101202501099001234500120010010000004561234567818",
  "mensaje": "Nota de Débito autorizada y PDF generado correctamente",
  "numeroAutorizacion": "3101202501099001234500120010010000004561234567818",
...
### Fallo — Nota de Débito rechazada por el SRI (DEVUELTA)
...
```json
{
  ...
  "claveAcceso": "3101202501099001234500120010010000004561234567818",
  "error": "Nota de Débito no autorizada por el SRI"
}
```
...
```json
{
  ...
  "mensajes": [],
  "claveAcceso": "3101202501099001234500120010010000004561234567818",
  "error": "La Nota de Débito aún está en procesamiento. Consulte la autorización más tarde con la clave de acceso."
}
```
...
6. **`motivos`** es una lista de objetos con `razon` (texto describiendo el motivo) y `valor` (monto en decimal). La suma de los valores de los motivos más el `totalSinImpuestos` debe ser consistente con el `valorTotal`.
7. El campo `infoAdicional` es opcional. Si se envía, cada elemento debe tener `nombre` y `valor` como strings. Campos comunes: `"Direccion"`, `"Telefono"`, `"Email"`.
